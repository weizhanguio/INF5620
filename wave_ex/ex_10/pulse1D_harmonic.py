...
def solver(I, V, f, c, U_0, U_L, L, Nx, C, T,
    ...
    # Special formula for the first step      2.0/(1/q[i]+1/q[ip1])        2.0/(1/q[i]+1/q[im1])  
    for i in range(1, Nx):
        u[i] = u_1[i] + dt*V(x[i]) + \
        0.5*C2*(2.0/(1/q[i]+1/q[i+1])*(u_1[i+1] - u_1[i]) - \
                2.0/(1/q[i]+1/q[i-1])*(u_1[i] - u_1[i-1])) + \
        0.5*dt2*f(x[i], t[0])
    ...
    if U_0 is None:
        ...
        im1 = ip1  # i-1 -> i+1
        u[i] = u_1[i] + dt*V(x[i]) + \
               0.5*C2*(2.0/(1/q[i]+1/q[ip1])*(u_1[ip1] - u_1[i])  - \
                       2.0/(1/q[i]+1/q[im1])*(u_1[i] - u_1[im1])) + \
        0.5*dt2*f(x[i], t[0])
    else:
        u[0] = U_0(dt)
    ...
    for n in range(1, N):
        ...
        if version == 'scalar':
            for i in range(1, Nx):
                u[i] = - u_2[i] + 2*u_1[i] + \
                    C2*(2.0/(1/q[i]+1/q[i+1])*(u_1[i+1] - u_1[i])  - \
                        2.0/(1/q[i]+1/q[i-1])*(u_1[i] - u_1[i-1])) + \
                dt2*f(x[i], t[n])
        ...
        if U_0 is None:
            ...
            im1 = ip1
            u[i] = - u_2[i] + 2*u_1[i] + \
                   C2*(2.0/(1/q[i]+1/q[ip1])*(u_1[ip1] - u_1[i])  - \
                       2.0/(1/q[i]+1/q[im1])*(u_1[i] - u_1[im1])) + \
            dt2*f(x[i], t[n])
        else:
            u[0] = U_0(t[n+1])
...
if __name__ == '__main__':
    import sys
    pulse(C=1, Nx=40, animate=True, version='scalar', T=2,
          loc='left', pulse_tp='plug', slowness_factor=4,
          medium=[0.7, 0.9], every_frame=1)
