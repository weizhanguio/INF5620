/*
title: Find a symmetry boundary condition, with Dirichlet boundary on the left
ex5  Page:  30
*/
#include<stdio.h>
#include<malloc.h>
#include<stdlib.h>
#include<math.h>
#pragma                   message("Please input  N_x(odd number),   N_t when run app")
#define   real    	      double
#define   u(i)              u[i]
#define   u_1(i)          u_1[i]
#define   u_2(i)          u_2[i]
...
int main(int nargs, char** args)
{
	...
	if (dt>=dx/(sqrt(c)))
	     {
	     	...
	     	exit(0);
		                                                            ...
		                                                            /* left boundary i=1, right boundary i=N_x*/	
	for(i=1;i<=N_x ;i++)		                         /*implement initial condition*/           
	    {
	...
	for(i=2;i<=N_x-1;i++)                                     /*first time step*/	  
		...
		u((N_x+1)/2+1)=u((N_x-1)/2)+b*dx;     	 /*update ghost points  for x=0*/
			temp=u_2;
			u_2=u_1;
			u_1=u;
			u=temp;					
